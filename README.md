...
- Server-side API routes at `app/api/logo/route.ts`, `app/api/image/route.ts`, `app/api/adult-image/route.ts`, `app/api/video/route.ts`, and `app/api/web-to-zip/route.ts`
...
- The image route keeps the original text-to-image provider, while `/api/adult-image` calls the separate 18+ provider with `GET /?prompt=...`.
